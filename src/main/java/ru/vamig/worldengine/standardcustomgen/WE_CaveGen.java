...
public class WE_CaveGen extends WE_CreateChunkGen {
	...
	public void gen(WE_GeneratorData data) {
		...
		for(long cx = data.chunk_X / 16L - (long)range; cx <= data.chunk_X / 16L + (long)range; ++cx)
			for(long cz = data.chunk_Z / 16L - (long)range; cz <= data.chunk_Z / 16L + (long)range; ++cz) {
				...
				for(int i = 0; i < e; ++i) {
					double
						gx = (double)cx * 16.0 + (double)rand.nextInt(16				   ),
						gy =					  (double)rand.nextInt(rand.nextInt(120) + 8),
						gz = (double)cz * 16.0 + (double)rand.nextInt(16				   );
					
					int k = 1;
					if(rand.nextInt(4) == 0) {
						caveGen_func(data, rand.nextLong(), gx, gy, gz, 1.0F + rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5);
						k += rand.nextInt(4);
					}
					//-//
					for(int l1 = 0; l1 < k; ++l1) {
						...
						caveGen_func(data, rand.nextLong(), gx, gy, gz, x2, x, x1, 0, 0, 1.0);
					}
				}
			}
	}
	...
	public void caveGen_func(WE_GeneratorData data, long rs, double gx, double gy, double gz, float fn1, float fn2, float fn3, int in1, int in2, double dn) {
		double cx = (double)data.chunk_X + 8.0, cz = (double)data.chunk_Z + 8.0;
		...
		for(boolean flag = r.nextInt(6) == 0; in1 < in2; ++in1) {
			double d6 = 1.5 + (double)MathHelper.sin((float)in1 * (float)Math.PI / (float)in2) * (double)fn1, d7 = d6 * dn;
			...
			if(!flag2 && in1 == k1 && fn1 > 1.0F && in2 > 0) {
				caveGen_func(data, r.nextLong(), gx, gy, gz, r.nextFloat() * 0.5F + 0.5F, fn2 - (float)Math.PI / 2.0F, fn3 / 3.0F, in1, in2, 1.0);
				caveGen_func(data, r.nextLong(), gx, gy, gz, r.nextFloat() * 0.5F + 0.5F, fn2 + (float)Math.PI / 2.0F, fn3 / 3.0F, in1, in2, 1.0);
				return;
			}
			
			if(flag2 || r.nextInt(4) != 0) {
				double d8 = gx - cx, d9 = gz - cz, d10 = (double)in2 - (double)in1, d11 = (double)fn1 + 18.0;
				if(d8 * d8 + d9 * d9 - d10 * d10 > d11 * d11)
					return;
				
				if(gx >= cx - 16.0 - d6 * 2.0 && gz >= cz - 16.0 - d6 * 2.0 && gx <= cx + 16.0 + d6 * 2.0 && gz <= cz + 16.0 + d6 * 2.0) {
					...
					if(!flag3) {
						for(k2 = i4; k2 < l1; ++k2) {
							double d13 = ((double)k2 + (double)data.chunk_X + 0.5 - gx) / d6;
							for(j3 = k4; j3 < j2; ++j3) {
								double d14 = ((double)j3 + (double)data.chunk_Z + 0.5 - gz) / d6;
								int k3 = (k2 * 16 + j3) * 256 + i2;
								if(d13 * d13 + d14 * d14 < 1.0)
									for(int l3 = i2 - 1; l3 >= j4; --l3) {
										double d12 = ((double)l3 + 0.5 - gy) / d7;
