...
public class WE_RavineGen extends WE_CreateChunkGen {
			...
			for(long cz = data.chunk_Z / 16L - (long)range; cz <= data.chunk_Z / 16L + (long)range; ++cz) {
				...
				if(rand.nextInt(50) == 0) {
					double
						...
						gy =					  (double)rand.nextInt(rand.nextInt(40) + 8) + 20.0,
					...
					for(int i1 = 0; i1 < b0; ++i1) {
						...
						caveGen_func(data, rand.nextLong(), gx, gy, gz, x2, x, x1, 0, 0, 3.0);
					}
				}
			}
	}
	...
	public void caveGen_func(WE_GeneratorData data, long rs, double gx, double gy, double gz, float fn1, float fn2, float fn3, int in1, int in2, double dn) {
		double d4 = (double)data.chunk_X + 8.0, d5 = (double)data.chunk_Z + 8.0;
		...
		for(; in1 < in2; ++in1) {
			double d12 = 1.5 + (double)MathHelper.sin((float)in1 * (float)Math.PI / (float)in2) * (double)fn1, d6 = d12 * dn;
			...
			if(flag1 || r.nextInt(4) != 0) {
				double d7 = gx - d4, d8 = gz - d5, d9 = (double)in2 - (double)in1, d10 = (double)fn1 + 18.0;
							...
							for(j3 = k4; j3 < j2; ++j3) {
								...
								if(d13 * d13 + d14 * d14 < 1.0)
									for(int l3 = i2 - 1; l3 >= j4; --l3) {
										...
										if((d13 * d13 + d14 * d14) * (double)field_75046_d[l3] + d11 * d11 / 6.0 < 1.0)
