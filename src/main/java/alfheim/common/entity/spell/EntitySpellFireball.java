...
public class EntitySpellFireball extends Entity implements ITimeStopSpecific {
	...
	public void onImpact(MovingObjectPosition mop) {
		if (!worldObj.isRemote) {
			if (mop != null && mop.entityHit != null) mop.entityHit.attackEntityFrom(DamageSourceSpell.fireball(this, caster), SpellBase.over(caster, 6));
			for (Object o : worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.getBoundingBox(posX, posY, posZ, posX, posY, posZ).expand(2, 2, 2))) {
				...
				if (!PartySystem.mobsSameParty(e, caster)) e.attackEntityFrom(DamageSourceSpell.fireball(this, caster), SpellBase.over(caster, 6));
			}
