...
public class TileElvenPylon extends TileEntity {
	...
	public void updateEntity() {
		...
		if(activated && worldObj.isRemote) {
			...
			if(ConfigHandler.elfPortalParticlesEnabled) {
				...
				Botania.proxy.wispFX(worldObj, x, yCoord + 0.25, z,  0.75F + (float) Math.random() * 0.25F, 0.5F + (float) Math.random() * 0.25F, (float) Math.random() * 0.25F, 0.25F + (float) Math.random() * 0.1F, -0.075F - (float) Math.random() * 0.015F);
				if(worldObj.rand.nextInt(3) == 0)
					Botania.proxy.wispFX(worldObj, x, yCoord + 0.25, z, 0.75F + (float) Math.random() * 0.25F, 0.5F + (float) Math.random() * 0.25F, (float) Math.random() * 0.25F, 0.25F + (float) Math.random() * 0.1F, (float) movementVector.x, (float) movementVector.y, (float) movementVector.z);
			}
		}

		if(worldObj.rand.nextBoolean() && worldObj.isRemote)
			Botania.proxy.sparkleFX(worldObj, xCoord + Math.random(), yCoord + Math.random() * 1.5, zCoord + Math.random(), 1.0F, 0.5F, 0F, (float) Math.random(), 2);
	}
}
