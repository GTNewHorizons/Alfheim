...
object CardinalSystem {
	...
	fun load(save: String) {
		...
		try {
			...
			TimeStopSystem.tsAreas = oin.readObject() as HashMap<Int, LinkedList<alfheim.common.core.handler.CardinalSystem.TimeStopSystem.TimeStopArea>>
			oin.close()
		} catch (e: Throwable) {
	...
	object SpellCastingSystem {
		...
		fun getCoolDown(caster: EntityPlayer, spell: SpellBase): Int {
			try {
				return forPlayer(caster).coolDown[spell]
			} catch (e: Throwable) {
		...
		class ManaSyncHandler {
			...
			fun onLivingUpdate(e: LivingUpdateEvent) {
				if (AlfheimCore.enableMMO && ASJUtilities.isServer && e.entityLiving is EntityPlayer) {
					...
					if (player.worldObj.totalWorldTime % 20 == 0L) ManaSystem.handleManaChange(player)
				}
			}
		}
	}
	...
	object PartySystem {
					...
					if (segment.party.isMember(e)) {
						for (i in 0 until segment.party.count) {
							if (segment.party.isPlayer(i)) {
								val mr = segment.party.get(i)
								if (mr is EntityPlayerMP) {
									AlfheimCore.network.sendTo(Message2d(UUID, e.getEntityId().toDouble(), segment.party.indexOf(e).toDouble()), mr as EntityPlayerMP)
								}
							}
						}
					}
		...
		class Party: Serializable, Cloneable {
			...
			private var members: Array<Member>? = null
			var count: Int = 0
			...
			fun isPlayer(i: Int): Boolean {
				return members!![i].isPlayer
			}
			
			fun isDead(i: Int): Boolean {
					...
					if (mr is EntityPlayer) {
						...
						for (j in 0 until count) {
							if (members!![j].isPlayer) {
								val e = get(j)
								if (e is EntityPlayer) (e as EntityPlayer).addChatMessage(ChatComponentText(String.format(StatCollector.translateToLocal("alfheimmisc.party.memberdied"), mr.commandSenderName)))
							}
						}
					}
				}
			...
			private fun removeSafe(mr: EntityLivingBase?): Boolean {
				...
				--count
				if (mr is EntityPlayer)
					setParty(mr as EntityPlayer?, Party((mr as EntityPlayer?)!!))
				while (id < count) {
					members[id] = members!![id + 1]
					id++
				}
			...
			private fun sendChanges() {
				if (ASJUtilities.isServer)
					for (i in 0 until count) {
						val e = get(i)
						if (e != null && members!![i].isPlayer && e is EntityPlayerMP)
							transfer(e as EntityPlayerMP)
					}
			}
			
			private fun sendMana(player: EntityPlayer, mana: Int) {
			...
			private fun sendDead(id: Int, d: Boolean) {
				for (i in 0 until count) {
					val e = get(i)
					if (e != null && members!![i].isPlayer && e is EntityPlayerMP)
	...
	object TimeStopSystem {
		...
		fun transfer(player: EntityPlayerMP, fromDim: Int) {
			if (tsAreas.containsKey(fromDim)) for (tsa in tsAreas[fromDim]) AlfheimCore.network.sendTo(Message1d(Message1d.m1d.TIME_STOP_REMOVE, tsa.id.toDouble()), player)
			if (tsAreas.containsKey(player.dimension)) for (tsa in tsAreas[player.dimension]) AlfheimCore.network.sendTo(MessageTimeStop(PartySystem.getUUIDParty(tsa.uuid), tsa.pos.x, tsa.pos.y, tsa.pos.z, tsa.id), player)
		}
		...
		fun stop(caster: EntityLivingBase) {
			...
			if (!tsAreas.containsKey(caster.dimension)) tsAreas[caster.dimension] = LinkedList()
		...
		fun affected(e: Entity?): Boolean {
			...
			for (tsa in tsAreas[e.dimension]) {
				if (Vector3.vecEntityDistance(tsa.pos, e) < 16) {
					...
					if (e is EntityLivingBase) {
						if (!PartySystem.sameParty(tsa.uuid, e as EntityLivingBase?)) return true
					} else {
						return true
					}
				}
		...
		fun affected(te: TileEntity?): Boolean {
			...
			if (!tsAreas.containsKey(te.worldObj.provider.dimensionId)) return false
		...
		private class TimeStopArea(caster: EntityLivingBase): Serializable {
			val pos: Vector3
			val uuid: UUID
			@Transient
			val id: Int
	...
	class PlayerSegment(player: EntityPlayer): Serializable {
		...
		private fun readObject(`in`: ObjectInputStream) {
			try {
				...
				while (size > 0) {
					coolDown[AlfheimAPI.getSpellInstance(`in`.readUTF())] = `in`.readInt()
					--size
				}
