...
/*class AlfheimClassTransformerKT : IClassTransformer {
    override fun transform(name: String, transformedName: String, basicClass: ByteArray): ByteArray {
        if (transformedName == "vazkii.botania.common.item.relic.ItemRelic") {
            ...
            val ct = ItemRelic_ClassVisitor(cw)
            cr.accept(ct, ClassReader.SKIP_FRAMES)
            return cw.toByteArray()
        }
        return basicClass
    ...
    class ItemRelic_ClassVisitor(cv: ClassVisitor) : ClassVisitor(ASM5, cv) {
        override fun visitMethod(access: Int, name: String, desc: String, signature: String, exceptions: Array<String>): MethodVisitor {
            if (name == "addBindInfo") return ItemRelic_addBindInfo_MethodVisitor(super.visitMethod(access, name, desc, signature, exceptions))
            return super.visitMethod(access, name, desc, signature, exceptions)
        }

        class ItemRelic_addBindInfo_MethodVisitor(mv: MethodVisitor) : MethodVisitor(ASM5, mv) {
            private var perform = false
            override fun visitJumpInsn(opcode: Int, label: Label) {
                if (opcode == IF_ACMPNE) {
                    if (!perform) {
