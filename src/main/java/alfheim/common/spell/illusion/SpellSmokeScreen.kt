...
class SpellSmokeScreen: SpellBase("smokescreen", EnumRace.SPRIGGAN, 5000, 600, 20) {
	...
	override fun performCast(caster: EntityLivingBase): SpellBase.SpellCastResult {
		...
		val list = caster.worldObj.getEntitiesWithinAABB(EntityLivingBase::class.java, caster.boundingBox.expand(16.0, 16.0, 16.0))
		for (living in list) {
			...
			AlfheimCore.network.sendToAll(MessageEffect(living.getEntityId(), Potion.blindness.id, 200, -1))
		}
