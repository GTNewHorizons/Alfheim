...
class SpellIgnition: SpellBase("ignition", EnumRace.SALAMANDER, 2000, 100, 5) {
	...
	override fun performCast(caster: EntityLivingBase): SpellCastResult {
		...
		val mana = caster.capabilities.isCreativeMode || consumeMana(caster, (getManaCost() * if (race == caster.race) 1.0 else 1.5).toInt(), false)
		...
		if (stackToPlace.stackSize == 0) {
			consumeMana(caster, (getManaCost() * if (race == caster.race) 1.0 else 1.5).toInt(), true)
