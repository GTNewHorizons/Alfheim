...
class SpellAquaBind: SpellBase("aquabind", EnumRace.UNDINE, 4000, 600, 15) {
	...
	override fun performCast(caster: EntityLivingBase): SpellBase.SpellCastResult {
		...
		if (mop == null || mop.typeOfHit == MovingObjectType.MISS) return SpellBase.SpellCastResult.WRONGTGT
		...
		val l = caster.worldObj.getEntitiesWithinAABB(EntityLivingBase::class.java, AxisAlignedBB.getBoundingBox(hit.x, hit.y, hit.z, hit.x, hit.y, hit.z).expand(3.5, 0.5, 3.5))
		for (e in l) {
			...
			if (hit.copy().sub(mob).length() <= 3.5) {
				...
				AlfheimCore.network.sendToAll(MessageEffect(e.getEntityId(), Potion.moveSlowdown.id, 100, 4))
			}
		}
	...
	override fun render(caster: EntityLivingBase) {
		...
		val y = if (mop.typeOfHit == MovingObjectType.BLOCK) 0.1 * (if (mop.sideHit == 0) -1 else 1) else 0
		val s = 3.5
