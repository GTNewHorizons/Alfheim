...
public class WorldChunkManagerAlfheim extends WorldChunkManager {
	public static ArrayList<BiomeGenBase> allowedBiomes = new ArrayList<BiomeGenBase>(
			Arrays.asList(DimensionUtil.alfheimBiome));
	...
	public WorldChunkManagerAlfheim(long seed, WorldType worldtype) {
		this();
		// GenLayer[] agenlayer = GenLayer.initializeAllBiomeGenerators(seed,
		// worldtype);
		GenLayer[] agenlayer = GenLayerAlfheim.makeTheWorld(seed, worldtype);
	...
	public WorldChunkManagerAlfheim(World world) {
	 ...
	 */
	public float[] getRainfall(float[] listToReuse, int x, int y, int w, int h) {
		IntCache.resetIntCache();

		if (listToReuse == null || listToReuse.length < w * h) {
			listToReuse = new float[w * h];
		}

		int[] aint = this.biomeIndexLayer.getInts(x, y, w, h);
