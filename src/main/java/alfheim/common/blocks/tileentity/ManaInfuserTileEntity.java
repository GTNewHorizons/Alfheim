...
public class ManaInfuserTileEntity extends TileMod implements ISparkAttachable {
	...
	private static final int[][] LAPIS_BLOCKS = {{1,0},{-1,0},{0,1},{0,-1}};
	...
	public static MultiblockSet makeMultiblockSet() {
		...
		for(int[] l : LAPIS_BLOCKS) mb.addComponent(l[0], 0, l[1], Blocks.lapis_block, 0);
			...
			if(areItemsValid(items)) {
				...
				if(mana >= manarequest && !worldObj.isRemote) {
					...
					for(EntityItem otherItem : items)
						if(otherItem != item)
							otherItem.setDead();
						else item.setEntityItemStack(result);
	...
	boolean areItemsValid(List<EntityItem> items) {
		boolean[] equalitylist;
		for (IManaInfusionRecipe recipe : ManaInfusionRecipies.recipes) {
			if(items.size() != recipe.ingredients.size()) continue; // Odd items will mess up the infusion, less means not enough materials
			
			equalitylist = new boolean[recipe.ingredients.size()]; // this array contains whether required ingredient is inside of AABB of infuser
			...
			for(EntityItem item : items) { // For every item in AABB
				...
				for (int i = 0; i < recipe.ingredients.size(); i++) {
					...
					if(stack.getItem() == ing.getItem() && size == ing.stackSize && meta == ing.getItemDamage()) equalitylist[i] = true; // Marking true for further processing
				}
			}
			...
			for (boolean deflag : equalitylist) { // But let's check
				flagAllEqual = deflag;
				if (!flagAllEqual) return false; // Oh no! Something went wrong!
	...
	boolean hasValidPlatform() {
		return checkAll(LAPIS_BLOCKS, Blocks.lapis_block, 0) && checkAll(ELEMENTIUM_BLOCKS, ModBlocks.storage, 2);
	}
