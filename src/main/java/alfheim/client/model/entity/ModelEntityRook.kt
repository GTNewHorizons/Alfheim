...
class ModelEntityRook: ModelBase() {
	...
	override fun setRotationAngles(time: Float, amplitude: Float, ticksExisted: Float, yawHead: Float, pitchHead: Float, size: Float, entity: Entity?) {
		...
		righ_arm1.rotateAngleX = MathHelper.cos(time * 0.6662f + Math.PI.toFloat()) * 2.0f * amplitude * 0.5f - Math.toRadians(7.0).toFloat()
		righ_arm1.rotateAngleZ = Math.toRadians(13.0).toFloat()
		lef_arm1.rotateAngleZ = Math.toRadians(-13.0).toFloat()
		righ_leg1.rotateAngleX = MathHelper.cos(time * 0.6662f) * 1.4f * amplitude + Math.toRadians(7.0).toFloat()
		lef_leg1.rotateAngleX = MathHelper.cos(time * 0.6662f + Math.PI.toFloat()) * 1.4f * amplitude + Math.toRadians(7.0).toFloat()
		righ_leg1.rotateAngleZ = Math.toRadians(9.0).toFloat()
		lef_leg1.rotateAngleZ = Math.toRadians(-9.0).toFloat()
	}
	...
	override fun setLivingAnimations(living: EntityLivingBase?, time: Float, amplitude: Float, ticksExisted: Float) {
		if (living is EntityRook) {
			...
			if (i > 0) {
				lef_arm1.rotateAngleX = -2.0f + 1.5f * interpolate(i.toFloat() - ticksExisted, 20.0f)
				lef_arm1.rotateAngleX /= -2.0f
				lef_arm1.rotateAngleX += Math.toRadians(16.0).toFloat()
			} else {
				lef_arm1.rotateAngleX = MathHelper.cos(time * 0.6662f) * 2.0f * amplitude * 0.5f + Math.toRadians(16.0).toFloat()
			}
		}
	}
