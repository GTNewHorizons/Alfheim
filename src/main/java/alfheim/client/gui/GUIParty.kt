...
class GUIParty(private val mc: Minecraft): Gui() {
	...
	fun onOverlayRendering(event: RenderGameOverlayEvent.Post) {
		...
		if (AlfheimConfig.selfHealthUI) {
			...
			// ################ health: ################
			run {
				val mod = (Math.min(player.health, player.maxHealth) / Math.max(player.maxHealth, 1f) * 158.0).toInt() / 158.0
			...
			mana@ run {
				...
				val col = Color(Color.HSBtoRGB(0.55f, if (anyRequest) Math.min(1.0, Math.sin(System.currentTimeMillis() / 1000.0) * 0.25 + 1f).toFloat() else 1, 1f))
			...
			run {
				...
				for (i in 0 until data.length) {
					val c = data[i]
					if ("\u00c0\u00c1\u00c2\u00c8\u00ca\u00cb\u00cd\u00d3\u00d4\u00d5\u00da\u00df\u00e3\u00f5\u011f\u0130\u0131\u0152\u0153\u015e\u015f\u0174\u0175\u017e\u0207\u0000\u0000\u0000\u0000\u0000\u0000\u0000 !\"#$%&\'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~\u0000\u00c7\u00fc\u00e9\u00e2\u00e4\u00e0\u00e5\u00e7\u00ea\u00eb\u00e8\u00ef\u00ee\u00ec\u00c4\u00c5\u00c9\u00e6\u00c6\u00f4\u00f6\u00f2\u00fb\u00f9\u00ff\u00d6\u00dc\u00f8\u00a3\u00d8\u00d7\u0192\u00e1\u00ed\u00f3\u00fa\u00f1\u00d1\u00aa\u00ba\u00bf\u00ae\u00ac\u00bd\u00bc\u00a1\u00ab\u00bb\u2591\u2592\u2593\u2502\u2524\u2561\u2562\u2556\u2555\u2563\u2551\u2557\u255d\u255c\u255b\u2510\u2514\u2534\u252c\u251c\u2500\u253c\u255e\u255f\u255a\u2554\u2569\u2566\u2560\u2550\u256c\u2567\u2568\u2564\u2565\u2559\u2558\u2552\u2553\u256b\u256a\u2518\u250c\u2588\u2584\u258c\u2590\u2580\u03b1\u03b2\u0393\u03c0\u03a3\u03c3\u03bc\u03c4\u03a6\u0398\u03a9\u03b4\u221e\u2205\u2208\u2229\u2261\u00b1\u2265\u2264\u2320\u2321\u00f7\u2248\u00b0\u2219\u00b7\u221a\u207f\u00b2\u25a0\u0000".indexOf(c.toInt()) == -1) {
						shadow = false
						break
					}
				}
				...
				font.drawString(data, 88 - font.getStringWidth(data) / 2, 3, if (CardinalSystemClient.segment!!.target === mc.thePlayer) G else if (pt!!.get(0) === mc.thePlayer) R else -0x1, shadow)
				glTranslated(0.0, 0.0, 89.0)
			}
			glColor4d(1.0, 1.0, 1.0, 1.0)
			glPopMatrix()
			...
			for (i in 0 until pt!!.count) {
				...
				run {
					...
					if (l == null) {
						...
						hp = Math.min(l!!.health, l!!.maxHealth)
						hpm = l!!.maxHealth
					}
				...
				health@ run {
					...
					val mod: Double
					if (hp != -1f && hpm != -1f) {
						mod = (hp / Math.max(hpm, 1f) * 100.0).toInt() / 100.0
						ASJUtilities.glColor1u(if (mod > 0.5) G else if (mod > 0.1) Y else R)
					} else {
						mod = 1.0
					...
					if (length < 2)
						drawTexturedModalRect(34, y + 17 + 2, 133, 137, 1, 4)
					else if (length == 2) {
				...
				mana@ run {
					...
					val length = Math.min(100, pt!!.getMana(i) / 10000)
					...
					if (length < 2)
						drawTexturedModalRect(34, y + 25, 133, 145, 1, 4)
					else if (length == 2) {
		...
		if (AlfheimConfig.targetUI && CardinalSystemClient.segment!!.target != null) {
			...
			var hp = Math.min(l!!.health, l.maxHealth)
			var hpm = l.maxHealth
			var col = -0x222223 // bg color
			var st = false
			var shadow = true
			...
			health@ run {
				...
				val mod = (hp / Math.max(hpm, 1f) * 200.0).toInt() / 200.0
				ASJUtilities.glColor1u(if (mod > 0.5) G else if (mod > 0.1) Y else R)
		...
		run {
			...
			if (AlfheimConfig.selfHealthUI) {
				...
				val mod = MathHelper.floor_double(Flight.get(mc.thePlayer)) / Flight.max()
				val time = Math.sin((mc.theWorld.totalWorldTime / 2).toDouble()) * 0.5
				glColor4d(1.0, 1.0, 1.0, if (mc.thePlayer.capabilities.isFlying) if (mod > 0.1) time + 0.5 else time else 1)
