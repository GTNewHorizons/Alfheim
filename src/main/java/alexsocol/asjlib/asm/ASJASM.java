...
public class ASJASM implements IClassTransformer {
	...
	public byte[] transform(String name, String transformedName, byte[] basicClass) {
		...
		for (FieldData fd : fieldsMap.get(transformedName)) cw.visitField(fd.access, fd.name, fd.desc, null, null).visitEnd();
		cr.accept(cw, 0);
	...
	public static void registerFieldHookContainer(String className) {
		try {
			transform(IOUtils.toByteArray(ASJASM.class.getResourceAsStream('/' + className.replace('.', '/') + ".class")));
		} catch (IOException e) {
