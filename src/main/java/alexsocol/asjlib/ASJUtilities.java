...
public class ASJUtilities {
	...
	private static final ArrayList<String> blockIconsNames = new ArrayList<String>();
	private static final ArrayList<String> itemsIconsNames = new ArrayList<String>();
	private static final Map<String, IIcon> blockIcons = new HashMap<String, IIcon>();
	private static final Map<String, IIcon> itemsIcons = new HashMap<String, IIcon>();
	...
	public static int getTrueDamage(ItemStack stack) {
		return Integer.valueOf(stack.toString().split("@")[1]);
	}
	...
	public static boolean isItemStackEqualNBT(ItemStack stack1, ItemStack stack2) {
		if(stack1 != null && stack2 != null && stack1.getItem() == stack2.getItem() && stack1.getItemDamage() == stack2.getItemDamage()) {
			if(!stack1.hasTagCompound() && !stack2.hasTagCompound()) {
				...
				return false;
			} else return stack1.stackTagCompound.equals(stack2.stackTagCompound);
		}
		return false;
	}
	...
	public static void removeRecipe(ItemStack resultItem) {
		Iterator<IRecipe> i = CraftingManager.getInstance().getRecipeList().iterator();
		while (i.hasNext()) {
			IRecipe r = i.next();
			if (ItemStack.areItemStacksEqual(resultItem, r.getRecipeOutput())) {
				FMLRelaunchLog.log("ASJLib", Level.INFO, "Removed Recipe: " + r + " -> " + r.getRecipeOutput());
				i.remove();
			}
		}
	}
	...
	public static MovingObjectPosition getMouseOver(EntityLivingBase entity, double dist, boolean interact) {
		...
		float f1 = 1.0F;
		List<Entity> list = entity.worldObj.getEntitiesWithinAABBExcludingEntity(entity, entity.boundingBox.addCoord(vec31.xCoord * dist, vec31.yCoord * dist, vec31.zCoord * dist).expand((double)f1, (double)f1, (double)f1));
		double d2 = d1;
	 ...
	 * Don't use it. Use {@link #getSelectedBlock(EntityLivingBase, double, boolean) getMouseOver} instead
	 */
	...
	public static void registerEntityEgg(Class<? extends Entity> entityClass, String name, int backColor, int frontColor, Object instance){
		...
		EntityRegistry.registerGlobalEntityID(entityClass, name, ID);
		EntityRegistry.registerModEntity(entityClass, name, ID, instance, 128, 1, true);
		EntityList.entityEggs.put(ID, new EntityList.EntityEggInfo(ID, backColor, frontColor));
	}
	...
	static {
		...
		if (isBadEffect != null) isBadEffect.setAccessible(true);
	}
	...
	public static boolean isBadPotion(Potion effect) {
		Boolean b = ASJReflectionHelper.getValue(isBadEffect, effect, false);
	...
	public static String toString(NBTTagCompound nbt) {
		...
		for (Object o : nbt.tagMap.entrySet()) {
			...
			if (v instanceof NBTTagList || v instanceof NBTTagCompound) {
				...
				else arr = toString((NBTTagCompound) v).split("\n");
	...
	public static String toString(NBTTagList nbt) {
		...
		for (Object obj : nbt.tagList) if (obj instanceof NBTTagList || obj instanceof NBTTagCompound) {
			...
			else ts = toString((NBTTagCompound) obj);
